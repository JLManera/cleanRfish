...
#' @param na.fill Logical indicating whether to interpolate missing values before smoothing
#'        (default = TRUE)
...
#' @export
smooth_path <- function(path_df, na.fill = TRUE, p = 3, n = 13) {
  # Create working copy and NA mask
  working_df <- path_df |>
    dplyr::arrange(time) |>
    dplyr::mutate(row_id = dplyr::row_number())

  mask <- is.na(working_df$x) | is.na(working_df$y)
  ...
  if (na.fill) {
    ...
    # Apply Savitzky-Golay smoothing to filled data
    working_df <- working_df |>
      dplyr::mutate(
    ...
    # Smooth only existing points and merge back
    ...
    working_df <- working_df |>
      dplyr::right_join(smoothed_df, by = "time")
  }
...
#' @param na.fill Logical passed to smooth_path() for NA handling
#' @param p Polynomial order for Savitzky-Golay filter (must be < n)
#' @param n Window size (number of points) for Savitzky-Golay filter (must be odd)
